...
MsgShow = 'ملكوت التفاعل استمر' 
elseif NumMsg > 150000 then 
MsgShow = 'ملك التلجرام' 
elseif NumMsg > 10000000 then
MsgShow = 'ملك تفاعل الكروبات'
elseif NumMsg > 10000000000 then
MsgShow = 'ما شفت تفاعل هيك'
end
return MsgShow 
end

function Getrtba(UserID,ChatID)
if UserID == 1331646237 Then
var = 'مطور السورس'
elseif UserID == our_id then 
var = 'بوت' 
...
if #monsha == 0 then 
message = message .."⇜ لا يوجد منشئيين !\n"
else
...
message = message..'*ـــــــــــــــــــــــــــــــــــــ \n\n⇜ قائمه المدراء :*\n\n'
...
if not msg.SudoUser and not service then return '⇜ أنـت لـسـت الـمـطـور\n✗' end
if msg.is_post_ then return "⇜ عذرا هذا بوت حمايه المجموعات وليس القنوات\n✗" end
if msg.type ~= "channel" then return '⇜ لا يمكنك تفعيل البوت في المجوعات العاديه / البوت يدعم فقط المجموعات الخارقه\n✗' end
if redis:get(max..'group:add'..msg.chat_id_) then  return '⇜ المجموعه بالتأكيد تم تفعيلها\n✓' end
...
{{text = "آشـترگ بآلقنآهہ ✓",url=t.me/"..UserChaneel.."}},
}
...
function modrem(msg)
if not msg.SudoUser then return '⇜ أنـت لـسـت الـمـطـور\n✗'end
...
if cmd == "iduser" then
return SendMention(ChatID,UserID,MsgID,"⇜ اضغط علي الايدي ليتم نسخه\n\n "..USERNAME.." » ❲ "..UserID.." ❳",37,USERCAR)
end
if cmd == "whois" then
...
if cmd == "iduser" then
return sendMsg(ChatID,MsgID,"⇜ اضغط علي الايدي ليتم نسخه\n\n "..UserName.." » ❲ `"..UserID.."` ❳")
end

if cmd == "whois" then
return sendMsg(ChatID,MsgID,
  '⇜ الاسم » ❲ '..FilterName(data.title_,30)..' ❳\n⇜ الايدي « ❲ '..UserID..' ❳\n⇜ المعرف « ❲ '..UserName..' ❳\n⇜ نوع الكشف « بالمعرف\n✓')
end
...
return sendMsg(ChatID,MsgID,"⇜ لآ يوجد عضو بهذا المعرف\n✗")
end 
...
elseif UserID == SUDO_ID then 
return sendMsg(ChatID,MsgID,"⇜ لا يمكنك طرد تقييد المطور الاساسي\n✗") 
elseif redis:sismember(max..':SUDO_BOT:',UserID) then 
return sendMsg(ChatID,MsgID,"⇜ لا يمكنك تقييد المطور\n✗") 
elseif redis:sismember(max..':MONSHA_BOT:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,"⇜ لا يمكنك تقييد المنشئ\n✗") 
